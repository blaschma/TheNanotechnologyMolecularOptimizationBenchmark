...
class IndexTokenizer:
    """Tokenizer for space-separated integer index sequences.

    Input:  "3 0 7 2 1"
    Output: ['^', '3', '0', '7', '2', '1', '$']

    The vocabulary indices come from Action_Space_GroupSelfies.reversed_action_space,
    which maps action names -> integers. We re-use those integers as token strings.
    """
...
def create_index_vocabulary(num_indices: int, tokenizer: "IndexTokenizer") -> "Vocabulary":
    """Creates a vocabulary for integer indices 0..num_indices-1.

    num_indices should equal Action_Space_GroupSelfies.N_actions so that every
    action index produced by encoding_to_action_sequence() has a corresponding token.

    Special REINVENT tokens '$' (end/pad, id=0) and '^' (start, id=1) are prepended.
    Action index tokens start at id=2.

    Args:
        num_indices : int  — should be action_space.N_actions
        tokenizer   : IndexTokenizer instance (kept for API consistency)
    """
    tokens = [str(i) for i in range(num_indices)]
    vocabulary = Vocabulary()
    vocabulary.update([STOP_TOKEN, START_TOKEN] + tokens)
    return vocabulary
