...
def _is_index_sequence(s: str) -> bool:
    """Check if string is a space-separated integer index sequence."""
    ...
    with open(filename, "r") as csvfile:
        ...
        for row in reader:
            ...
            if isinstance(columns, int):
                ...
                #print(f"DEBUG read: '{smiles}'")  # ← add
                orig_smiles = smiles
                if actions:
                    for action in actions:
                        if callable(action) and smiles:
                            smiles = action(orig_smiles)
                            #print(f"DEBUG after action: '{smiles}'")  # ← add
                if not smiles:
                    #print(f"DEBUG: smiles is None/empty after actions, skipping")  # ← add
                    continue
                smiles = smiles.replace(".", "|")
                is_idx = _is_index_sequence(smiles)
                skip = os.environ.get("REINVENT_SKIP_FILTERS", "0") == "1"
                #print(f"DEBUG: is_index={is_idx}, skip={skip}")  # ← add
                if not (skip and is_idx):
